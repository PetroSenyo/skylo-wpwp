...
class ia extends We {
    async init() {
        super.init(), this.sectionEl = document.querySelector(".truck-step-1"), this.scrollProgress = document.querySelector(".truckscene-progress"), this.scene = document.querySelector(".truckscene"), this.sheet = z.theatre.sheet("truck"), this.symbol1 = document.querySelector(".truckscene-signal-bar-symbol-1"), this.symbol2 = document.querySelector(".truckscene-signal-bar-symbol-2"), this.el.style.opacity = .01, this.initBg(), this.initText(), await oe(100), this.el.style.opacity = 0, this.observe()
    }

    initBg() {
        const e = {
            alpha: 0,
            bg: {x: 0, night: 0, day: {scale: 1}, fog: 0, scale: 0},
            forGround: {y: 0},
            signals1: {scale: 0, opacity: 0},
            signals2: {scale: 0, opacity: 0},
            satellite: {x: 0, y: 0, rotate: 0},
            bars: {
                fill: {r: 0, g: 0, b: 0},
                bar1: {opacity: 1},
                bar2: {opacity: 1},
        ...
        this.bg = {
            animation: this.sheet.object("bg", e),
            el: this.get(".truckscene-bg"),
            layer1: this.get(".truckscene-bg-layer-1"),
            layer2: this.get(".truckscene-bg-layer-2"),
            layer3: this.get(".truckscene-bg-layer-3"),
            layer4: this.get(".truckscene-bg-layer-4"),
            signals1El: this.get(".truck-scene-signals-1"),
            signals2El: this.get(".truck-scene-signals-2"),
            bgNight: this.get(".truckscene-bg-night"),
            bgDay: this.get(".truckscene-bg-day"),
            fog: this.get(".truckscene-fog"),
            satelliteEl: this.get(".truckscene-satellite"),
            ground: this.get(".truckscene-forground"),
            barsEl: {
                el: this.get(".truckscene-signal-bar"),
                bar1: this.get(".truckscene-signal-bar-1"),
                bar2: this.get(".truckscene-signal-bar-2"),
                bar3: this.get(".truckscene-signal-bar-3"),
                bar4: this.get(".truckscene-signal-bar-4")
            }
        }, this.bg.sequence = this.sheet.sequence;
        const {el: t, bg: i} = this, {
            layer1: s,
            layer2: o,
            layer3: a,
            layer4: l,
        ...
        this.bg.animation.onValuesChange(function ({alpha: O, bg: x, signals1: E, signals2: b, bars: R, satellite: V}) {
            De(s, x.x, "em"), De(o, x.x, "em"), De(a, x.x, "em"), De(l, x.x, "em"), t.style.transform = `scale(${x.scale})`, v.style.opacity = x.fog, m.style.transform = `scale(${x.day.scale})`, n.style.transform = `scale(${E.scale})`, n.style.opacity = E.opacity, h.style.transform = `scale(${b.scale})`, h.style.opacity = b.opacity, f.bar1.style.opacity = R.bar1.opacity, f.bar2.style.opacity = R.bar2.opacity, f.bar3.style.opacity = R.bar3.opacity, f.bar4.style.opacity = R.bar4.opacity, f.el.style.color = `rgb(${R.fill.r}, ${R.fill.g}, ${R.fill.b})`, S.style.transform = `translate(${V.x}em,${V.y}em) rotate(${V.rotate}deg)`
        })
    }

    initText() {
        const e = this.get(".truckscene-text-part-1"), t = this.get(".truckscene-text-part-2"),
            i = this.get(".truckscene-text-part-3"), s = this.get(".truckscene-text-progress-bar"),
            o = {progress: 0, text1: {opacity: 0, y: 0}, text2: {opacity: 0, y: 0}, text3: {opacity: 0, y: 0}};
        this.textSheet = this.sheet.object("text", o), this.textSheet.onValuesChange(function ({
                                                                                                   text1: l,
                                                                                                   text2: n,
                                                                                                   text3: h,
                                                                                                   progress: f
                                                                                               }) {
            e.style.opacity = l.opacity, W(e, l.y, "em"), t.style.opacity = n.opacity, W(t, n.y, "em"), i.style.opacity = h.opacity, W(i, h.y, "em"), s.style.transform = `scaleX(${f})`
        })
    }

    onScroll(e) {
        if ((e.id !== "TRUCK-STEP-1" || e.id !== "TRUCK-STEP-2" || e.id !== "TRUCK-STEP-3" || e.id !== "GLOBE-STEP-2") && (this.el.style.opacity = 0), e.id === "TRUCK-STEP-1") {
            if (!e.inView) return;
            this.el.style.opacity = I(e.progress, .35, 1, .001, 1), this.bg.sequence.position = I(e.progress, .35, 1, .001, 2)
        }
        if (e.id === "TRUCK-STEP-2") {
            if (!e.inView) return;
            this.el.style.opacity = 1, this.bg.sequence.position = I(e.progress, 0, 1, 2, 4)
        }
        if (e.id === "TRUCK-STEP-3") {
            if (!e.inView) return;
            this.el.style.opacity = 1, this.bg.sequence.position = I(e.progress, 0, 1, 4, 6), this.symbol1.style.opacity = I(e.progress, .5, .75, 1, 0), this.symbol2.style.opacity = I(e.progress, .75, 1, 0, 1)
        }
...
class na extends We {
    ...
    onScroll(e) {
        if (!!T.gl && ((e.id !== "GLOBE-STEP-1" || e.id !== "TRUCK-STEP-1") && (this.el.style.opacity = 0), e.id === "GLOBE-STEP-1" && T.gl.camera.center.copy(K.w < H.BREAKPOINTS.MD ? T.gl.camera.centerDefaults.mobile : T.gl.camera.centerDefaults.dekstop), e.id === "TRUCK-STEP-1")) {
            if (!e.inView) return;
            const t = I(e.progress, 0, 1, 1, 3);
            T.gl.globe.scale.set(t, t, t), T.gl.setOpacity(I(e.progress, 0, .5, 1, 0)), W(this.text, I(e.progress, 0, 1, 0, -300)), this.text.style.opacity = fe(I(e.progress, 0, .5, 1, 0), 0, 1), this.markers.style.opacity = fe(I(e.progress, 0, .5, 1, 0), 0, 1)
        }
    }
}
